...
<div>
    ...
    <a href="({t_url m=ktai a=page_f_home})&amp;target_c_member_id=({$item.c_member_id})&amp;({$tail})">({$item.nickname|t_body:'name'})</a>({if $item.nickname_to})&nbsp;>>({$item.nickname_to|t_body:'name'})&nbsp;({/if})「({$item.comment|t_body:'title'})」<span style="text-size:small">･･･({$item.r_datetime|t_date})</span>
    ({if $item.c_member_id == $u})&nbsp;<a href="./?m=ktai&amp;a=do_h_one_word_delete&amp;c_one_word_id=({$item.c_one_word_id})&amp;({$tail})">削除</a>&nbsp;({/if})

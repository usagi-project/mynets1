...
<div>
    ...
    <a href="({t_url m=ktai a=page_f_home})&amp;target_c_member_id=({$item.c_member_id})&amp;({$tail})">({$item.nickname|t_body:'name'})</a>({if $item.nickname_to})&nbsp;>>({$item.nickname_to|t_body:'name'})&nbsp;({/if})「({$item.comment})」<span style="text-size:small">･･･({$item.r_datetime|t_date})</span>

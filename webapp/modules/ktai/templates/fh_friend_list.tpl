...
({$target_c_member.nickname|t_body:'name'}) さんの({$WORD_FRIEND_HALF})一覧<br>
<hr>

({foreach from=$target_friend_list item=target_friend})
<!--*VV matrix_L VV*-->
<a href="({t_url m=ktai a=page_f_home})&amp;target_c_member_id=({$target_friend.c_member_id})&amp;({$tail})">({$target_friend.nickname|t_body:'name'})</a>(({$target_friend.friend_count}))
<br>
({/foreach})

...
管理人:<a href="({t_url m=ktai a=page_f_home})&amp;target_c_member_id=({$c_commu.c_member_id_admin})&amp;({$tail})">({$c_commu.c_member_admin.nickname|t_body:'name'})</a><br>
<br>
■ｺﾐｭﾆﾃｨﾒﾝﾊﾞｰ(({$c_commu.count_member}))<br>
({foreach from=$c_commu_member_list item=c_commu_member })
<a href="({t_url m=ktai a=page_f_home})&amp;target_c_member_id=({$c_commu_member.c_member_id})&amp;({$tail})">({$c_commu_member.nickname|t_body:'name'})<br>
({/foreach})
<br>

...
<!-- ********************************* -->
<!-- ******ここから：コメント一覧****** -->
({**t_form m=pc a=page_fh_delete_dengon_comment**})
...
({$c_member.nickname|t_body:'name'})の伝言板
...
({if $is_prev})
<a href="({t_url m=pc a=page_fh_dengon})&amp;target_c_member_id=({$target_c_member_id})&amp;direc=-1&amp;page=({$page})">前を表示</a>
({/if})
...
件を表示
({if $is_next})
<a href="({t_url m=pc a=page_fh_dengon})&amp;target_c_member_id=({$target_c_member_id})&amp;direc=1&amp;page=({$page})">次を表示</a>
({/if})
...
<!--<input type="checkbox" name="target_c_dengon_comment_id[]" value="({$item.c_dengon_comment_id})" class="no_bg">-->
...
<a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$item.c_member_id_from})"><span class="DOM_fh_diary_comment_writer">({$item.nickname|t_body:'name'|default:"&nbsp;"})</span></a>
({else})
&nbsp;
({/if})
...
<input type="hidden" name="a" value="page_fh_dengon_comment_confirm">
<input type="hidden" name="target_c_member_id" value="({$target_c_member_id})">
...
<!-- ここまで：小タイトル --><div ID="entry_comment"></div>
({*ここまで：header*})
({*ここから：body*})
<!-- ここから：主内容 -->
<table border="0" cellspacing="0" cellpadding="0" style="width:524px;" class="border_01">
...
<td class="bg_01" align="center" style="width:1px;"><img src="./skin/dummy.gif" style="width:1px;height:1px;" class="dummy"></td>
<td class="bg_02" align="left" valign="middle">
<div style="padding:4px 3px;">

<textarea ID="MyForm" name="body" rows="8" cols="40" style="width:419px">({$requests.body})</textarea>
({*********絵文字*********})
({ext_include file="new_templates/emojipat_dengon.tpl"})
({*********絵文字*********})

...
<!-- **************************************** -->
<!-- ******ここから：コミュニティ新規作成****** -->
({t_form _enctype=file m=pc a=page_h_com_add_confirm _name=editForm})
...
<input type="radio" class="no_bg" name="open_flag" value="1"({if $form_val.open_flag == 1}) checked="checked"({/if})>公開する<br>
<input type="radio" class="no_bg" name="open_flag" value="0"({if $form_val.open_flag != 1}) checked="checked"({/if})>公開しない<br>
...
<textarea name="body" rows="6" cols="50">({$form_val.body})</textarea>
({* BBCode *})
...
({*<span>写真のアップロードに関するヘルプは <a href="" target="_blank">こちら</a></span>*})

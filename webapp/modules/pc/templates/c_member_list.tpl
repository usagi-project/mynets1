...
({if $item!=$page})<a href="({t_url m=pc a=page_c_member_list})&amp;page=({$item})&amp;target_c_commu_id=({$c_commu_id})">({$item})</a>
({else})({$item})
({/if})
 ({/foreach})]
...
({if $is_prev})<a href="({t_url m=pc a=page_c_member_list})&amp;direc=-1&amp;page=({$page})&amp;target_c_commu_id=({$c_commu_id})">＜前</a>({/if})　
({$start_num})件～({$end_num})件を表示
({if $is_next})<a href="({t_url m=pc a=page_c_member_list})&amp;direc=1&amp;page=({$page})&amp;target_c_commu_id=({$c_commu_id})">次＞</a>({/if})
...
<a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$item.c_member_id})"><img src="({t_img_url filename=$item.image_filename w=76 h=76 noimg=no_image_small})" class="pict"></a>
({else})
...
({if $item})
<a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$item.c_member_id})">({$item.nickname|t_body:'name'|default:"&nbsp;"}) (({$item.friend_count}))</a>
({else})
&nbsp;
({/if})
<img src="./skin/dummy.gif" class="v_spacer_m">

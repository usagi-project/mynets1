...
<td style="width:398px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">伝言内容を削除します</span></td>
...
({foreach from=$target_dengon_comment_list item=target_dengon_comment})
({*********})
<tr>
...
({$target_dengon_comment.r_datetime|date_format:"%Y年%m月%d日<br>%H:%M"|default:"&nbsp;"})
</div>
...
({if $target_dengon_comment.nickname})
<a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$target_dengon_comment.c_member_id})">({$target_dengon_comment.nickname|t_body:'name'|default:"&nbsp;"})</a>
({else})
&nbsp;
({/if})
<br>
({$target_dengon_comment.body|t_body:'dengon'|default:"&nbsp;"})<br>
...
({t_form m=pc a=do_fh_dengon_delete_c_dengon_comment})
...
<input type="hidden" name="target_c_member_id" value="({$target_c_member_id_to})">

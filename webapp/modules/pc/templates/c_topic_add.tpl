...
<!-- ******************************************** -->
<!-- ******ここから：新しいトピックを作成する****** -->
({t_form _enctype=file m=pc a=page_c_topic_add_confirm _name=editForm})
...
<td style="width:598px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">[({$c_commu.name|t_body:'name'})] 新規トピック作成</span></td>
</tr>
</table>
</div>
<!-- ここまで：小タイトル -->
...
({if $smarty.const.MYNETS_OPEN_COMMU && $c_commu.open_flag})
...
<input type="radio" class="no_bg" name="open_flag" value="1"({if $open_flag == 1}) checked="checked"({/if})>公開する<br>
<input type="radio" class="no_bg" name="open_flag" value="0"({if $open_flag == 0}) checked="checked"({/if})>公開しない<br>

※トピックを外部へ公開します。（外部からはコメント入力はできません）

...
<!-- ************************************** -->
<!-- ******ここから：メッセージテーブル****** -->
({t_form m=pc a=do_c_event_invite})
...
<td style="width:528px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">イベントお知らせメッセージを送る</span></td>
</tr>
</table>
<!-- ここまで：小タイトル -->
({*ここまで：header*})
...
<td style="width:564px;" class="bg_09" align="left" colspan="3">

<div class="padding_w_s lh_120">

({if $c_friend_list})
このイベントを紹介したい({$WORD_MY_FRIEND})を一覧から選び、紹介メッセージを書いてください。
({else})
...
<td style="width:1px;" class="bg_01" align="center"><img src="./skin/dummy.gif" style="width:1px;height:1px;" class="dummy"></td>
<td style="width:412px;" class="bg_02" align="left" valign="middle">
...
({counter assign=_cnt})
({if $_cnt % 3 == 1})<tr>({/if})
    <td><input type="checkbox" id="m({$c_friend.c_member_id})" name="c_member_id[]" value="({$c_friend.c_member_id})" class="no_bg"></td>
    <td style="padding-right:1em"><label for="m({$c_friend.c_member_id})">({$c_friend.nickname|t_body:'name'})</label></td>
({if $_cnt % 3 == 0})</tr>({/if})
({/foreach})
({if $_cnt % 3 != 0})</tr>({/if})
</table>

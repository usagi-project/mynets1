...
<!-- ************************************ -->
<!-- ******ここから：この人を紹介する****** -->
({t_form m=pc a=do_f_invite_insert_c_message_friend_invite})
...
<img src="({t_img_url filename=$target_member.image_filename w=76 h=76 noimg=no_image_small})">
...
({$target_member.nickname|t_body:'name'})
...
({if $_cnt % 3 == 1})<tr>({/if})
    ...
    <td style="padding-right:1em"><label for="m({$c_invite.c_member_id})">({$c_invite.nickname|t_body:'name'})</label></td>
({if $_cnt % 3 == 0})</tr>({/if})
({/foreach})
({if $_cnt % 3 != 0})</tr>({/if})
</table>
...
<textarea name="body" rows="5" cols="50">({$target_member.nickname|t_body:'name'})さんを紹介します。</textarea>

...
<input type="hidden" name="a" value="page_({$hash_tbl->hash('ext_topic_comment_check')})">
...
<br>
({if $pager.prev_page})
<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_topic_comment_check')})&amp;page=({$pager.prev_page})&amp;page_size=({$pager.page_size})">前へ</a>&nbsp;
({/if})
({foreach from=$pager.disp_pages item=i})
({if $i == $pager.page})
&nbsp;<strong>({$i})</strong>&nbsp;
({else})
<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_topic_comment_check')})&amp;page=({$i})&amp;page_size=({$pager.page_size})">&nbsp;({$i})&nbsp;</a>
({/if})
({/foreach})
({if $pager.next_page})
&nbsp;<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_topic_comment_check')})&amp;page=({$pager.next_page})&amp;page_size=({$pager.page_size})">次へ</a>
({/if})
</div>
<!-- pager_end -->
...
<div id="">
    <table>
        ...
        <tr>
            <td class="idnumber"><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_topic_comment_data')})&amp;target_c_commu_topic_comment_id=({$item.c_commu_topic_comment_id})">({$item.c_commu_topic_comment_id})</a></td>
            ...
            <td style="width:150px"><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_topic_data')})&amp;target_c_commu_topic_id=({$item.c_commu_topic_id})">({$item.topicname|t_body:'title'})</a></td>

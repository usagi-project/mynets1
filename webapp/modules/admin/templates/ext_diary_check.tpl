...
<input type="hidden" name="a" value="page_({$hash_tbl->hash('ext_diary_check')})">
...
<br>
({if $pager.prev_page})
<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_diary_check')})&amp;page=({$pager.prev_page})&amp;page_size=({$pager.page_size})">前へ</a>&nbsp;
({/if})
({foreach from=$pager.disp_pages item=i})
({if $i == $pager.page})
&nbsp;<strong>({$i})</strong>&nbsp;
({else})
<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_diary_check')})&amp;page=({$i})&amp;page_size=({$pager.page_size})">&nbsp;({$i})&nbsp;</a>
({/if})
({/foreach})
({if $pager.next_page})
&nbsp;<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('ext_diary_check')})&amp;page=({$pager.next_page})&amp;page_size=({$pager.page_size})">次へ</a>
({/if})
</div>
<!-- pager_end -->
...
<div id="">
    <table>
        ...
        <tr>
            <td class="idnumber"><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_diary_data')})&amp;target_c_diary_id=({$item.c_diary_id})">({$item.c_diary_id})</a></td>
            ...
            <td style="width:150px"><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_diary_data')})&amp;target_c_diary_id=({$item.c_diary_id})">({$item.subject|t_body:'title'})</a></td>
            <td style="width:300px">({$item.body|t_truncate:"120"|t_url2a|t_moji})</td>

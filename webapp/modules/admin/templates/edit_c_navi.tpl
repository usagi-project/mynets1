...
<h2>SNS設定：ナビゲーション変更</h2>

<p>ナビゲーション項目の「URL」と「説明(alt属性)」を変更することができます。</p>

<p class="caution">※「説明」を変更しても画像内の文字は自動的には変更されません。<br>
画像の変更は「<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_skin_image')})">スキン画像変更</a>」からおこなってください。<br>
スキン画像の変更は、skinディレクトリ以下にディレクトリ単位で作成したスキンをサーバーに保存することで、ディレクトリ切り替えで画像を変更します。</p>

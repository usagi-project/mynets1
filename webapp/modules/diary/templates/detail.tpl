...
({foreach from=$new_own_diary_list item=item})

<div><a href="({t_url m=diary a=page_detail})&amp;target_c_diary_id=({$item.c_diary_id})"><img src="./skin/dummy.gif" style="width:14px;height:14px;" class="icon icon_1">({$item.subject|t_body:'title'})</a></div>
...
<tr>
<td style="width:36px;" class="bg_06"><img src="({t_img_url_skin filename=content_header_1})" style="width:30px;height:20px;" class="dummy"></td>
<td style="width:111px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">全体の新着</span></td>
</tr>
</table>
({*ここまで：header*})
({*ここから：body*})
...
<a href="({t_url m=diary a=page_list})&amp;target_c_member_id=({$target_member.c_member_id})">({$target_member.nickname|t_body:'title'})</a>さんの公開日記
</span></td>
</tr>
</table>
<!-- ここまで：小タイトル -->
...
({$target_diary.r_datetime|date_format:"%m月%d日<br> %H:%M"})
...
<a href="({t_url m=diary a=page_detail})&amp;target_c_diary_id=({$target_diary.c_diary_id})">({$target_diary.subject|t_body:'title'})</a>
...
({$target_diary.body|t_body:'diary'})
</div>

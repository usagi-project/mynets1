 ...
 * @access public
 */
if (! function_exists('PictCount'))
{
    function PictCount($str)
    {
        $moji_pattern = '/\[([ies]):([0-9]{1,3})\]/i';
        $emoji_count  = preg_match_all($moji_pattern,$str,$emoji);
        $contents_count1 = mb_strlen(implode('', $emoji[0]));

        $moji_pattern = '/&(?:amp;|)#x([0-9A-F][0-9A-F][0-9A-F][0-9A-F]);/i';
        $count        = preg_match_all($moji_pattern,$str,$emoji);
        $contents_count2 = mb_strlen(implode('', $emoji[0]));

        $moji_pattern = '/\x1b\x24(\C\C)\x0f/';
        $counts       = preg_match_all($moji_pattern,$str,$emojis);
        $contents_count3 = mb_strlen(implode('', $emojis[0]));

        return (int)$contents_count1 + (int)$contents_count2 + (int)$contents_count3;
    }
}

<div style="clear:left;padding:1px;border:#eee 1px solid;">
    ...
    <div class="panel" id="tab1">
            ...
            <div style="padding:3px 5px;({if !$smarty.foreach.diary.last})border-bottom:#cccccc 1px dotted;({/if})">
                ...
                <a href="({$smaty.const.OPENPNE_URL})?m=diary&amp;a=page_detail&amp;target_c_diary_id=({$item.c_diary_id})">({$item.subject|bbcode2del|t_truncate:"16"|t_body:'title'})</a>
            </div>
            ({/foreach})
    </div>
    <div class="panel" id="tab2">
            ...
            <div style="padding:3px 5px;({if !$smarty.foreach.commu.last})border-bottom:#cccccc 1px dotted;({/if})">
                ...
                <a href="({$item.url})" class="top">({$item.info|bbcode2del|t_truncate:"16"|t_body:'title'})</a>
            </div>
            ({/foreach})
    </div>
</div>
